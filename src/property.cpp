...
Property::Property(int _particle_number, double _time_interval, string _output_path):
...
void Property::initalize(vector<Particle>& ensemble) {
    for (auto it = ensemble.begin(); it != ensemble.end(); ++it) {
        Start_status.push_back(*it);
    }
}

double Property::calc_mean_square_particle_displacement(vector<Particle>& ensemble) {
    double MSD(0);
...
void Property::sample_velocity_autocorrelation(vector<Particle> &ensemble) {
...
void Property::calc_velocity_autocorrelation() {
...
void Property::velocity_autocorr_output() {
